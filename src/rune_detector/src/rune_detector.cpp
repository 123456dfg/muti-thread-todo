...
namespace rune
{
    ...
    static std::unordered_map<int, fyt::EnemyColor> DNN_COLOR_TO_ENEMY_COLOR = {{0, fyt::EnemyColor::BLUE},
                                                                                {1, fyt::EnemyColor::RED}};
    ...
    bool RuneDetector::process(const cv::Mat resized_img, Eigen::Matrix3f transform_matrix, int64_t timestamp_nanosec, cv::Mat &src_img)
    {
    ...
    bool RuneDetector::infer(cv::Mat &src, int64_t timestamp_nanosec)
    {
        ...
        return process(resized_img, transform_matrix, timestamp_nanosec, src);
    }
